...
import { WriteElecRow } from "./fetchRoomMemberData";

export default async function downloadInitialElecExcel(rows: WriteElecRow[]) {
  // 分離 B 棟和 C 棟資料
  ...
  for (let i = 0; i < maxRows; i++) {
    ...
    const row = [
      cRoom?.roomNumber ?? "", cRoom?.name_1 ?? "", cRoom?.name_2 ?? "", "", "",
      bRoom?.roomNumber ?? "", bRoom?.name_1 ?? "", bRoom?.name_2 ?? "", "", "",
    ];
